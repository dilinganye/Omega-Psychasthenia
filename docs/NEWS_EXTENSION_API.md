...
- `headline` 直接作为左下角推送及 Intel 列表标题，不再自动添加“边缘新闻：”前缀。
- 左下角卡片的摘要固定为 `内容来源：<source>`；报道正文仍由 `report` 提供。
- `icon` 是可选的模组相对贴图路径，例如 `graphics/icons/intel/my_news.png`。留空时回退到新闻 Intel 当前势力的徽记；路径无效时也会安全回退并写入日志。
- 订阅是每个存档独立保存的玩家选项，默认开启。关闭只抑制新文章的左下角 Intel 推送；文章仍会静默进入“新闻”分类，十日过期、调查、现场和战略影响均照常运行。
- 外部代码可调用 `PTSDCrisisAPI.isNewsIntelSubscribed()` 与 `setNewsIntelSubscribed(boolean)` 查询或修改订阅状态。
...
`onIncidentCreated(IncidentContext)` 对不可调查新闻会在新闻卡创建后调用；对可被证伪的新闻则只在该新闻的隐藏结果确认为属实时调用。它可创建舰队或实体、修改公开的危机状态，并可返回一个实体作为调查目标。这样假消息不会提前泄漏或产生实体/战略副作用。
...
1. 玩家在 RECON 及以后记录新闻时按25%属实、70%假消息、5%跟踪舰差分；DORMANT 为25%属实、75%普通假消息，不生成第四窥视实体。Je专属实地任务固定拥有可验证现场。
...
- 可调查新闻创建时只施加CSV中的局部 `panic`，不会立刻写入侦察进度、人类认知、攻击性、现实扭曲、星系攻击权重或火力侦察战略事件。
- 只有抽取为属实的新闻会在玩家进入目标星系后实体化 `siteTemplates`/`siteHandler` 现场。
- 玩家抵近并确认属实后保留原有恐慌，其余战略数值与延迟物理影响按CSV原值的50%结算。
- 玩家抵近并证伪后，系统按该事件保存的逐殖民地贡献撤销恐慌；其他战略影响从未生效。
- 旧存档中由旧版代码创建的新闻缺少生命周期版本标记。它们被视为已经按旧规则结算，不会在热更新后重复增加影响或错误回滚历史数值。
