...
public class MissionDefinition implements MissionDefinitionPlugin {
    ...
    public void defineMission(MissionDefinitionAPI api) {
        ...
        api.initFleet(FleetSide.PLAYER, "绿方", FleetGoal.ATTACK, false, 5);
        api.initFleet(FleetSide.ENEMY, "红方", FleetGoal.ATTACK, false, 5);

        api.setFleetTagline(FleetSide.PLAYER, "绿方舰队");
        api.setFleetTagline(FleetSide.ENEMY, "红方舰队");

        api.addBriefingItem("此测试将会自动启用开发者模式 (dev mode)");
    ...
    private final static class Plugin extends BaseEveryFrameCombatPlugin {
        ...
        public void advance(float amount, List<InputEventAPI> events) {
            ...
            while (playerDP < battleSize / 2) {
                ...
                log.info(String.format("生成 %s", member.getSpecId()));
            ...
            if (Global.getCombatEngine().getFleetManager(FleetSide.ENEMY).getTaskManager(false).isInFullRetreat()
                ...
                if (!ended) {
                    ...
                    log.info(String.format("基准测试结果 (运行时间 %.1f 分钟):", (System.currentTimeMillis() - epoch) / 60000.0));
                    log.info(String.format("  平均帧率: %.1f", fpsAvg));
                    log.info(String.format("  最低帧率: %.1f", minimumFPS));
                    log.info(String.format("  平均帧时间波动: %.2fms", variancesAvg));
                    log.info(String.format("  体感游戏速度: %d%%", Math.round(gameSpeed * 100.0)));
                    log.info(String.format("  战场规模: %d", battleSize - 200));
                    log.info(String.format("********************************"));
                    Global.getSoundPlayer().playUISound("cr_playership_critical", 1f, 2f);
                }
                Global.getCombatEngine().addFloatingText(new Vector2f(0f, 0f), String.format("推荐的战场规模: %d", battleSize - 200), 200f, Color.yellow, null, 1f, 0f);
                return;
            }
            ...
            if (System.currentTimeMillis() - epochShort >= 1000) {
                ...
                double intervalAvg = (System.currentTimeMillis() - epoch) / 1000.0;
                double fpsAvg = frames / intervalAvg;
                log.info(String.format("基准测试 [%s] - %.1f FPS - 内存用量 %.2fGB", new SimpleDateFormat("HH.mm.ss").format(new Date()), fps, usedMemGB));
                Global.getCombatEngine().addFloatingText(new Vector2f(0f, 87.5f), String.format("进度: %d%%", (int) (progress * 100.0)), 75f, Color.yellow, null, 1f, 0f);
                Global.getCombatEngine().addFloatingText(new Vector2f(0f, 0f), String.format("平均帧率: %.1f", fpsAvg), 100f, Color.yellow, null, 1f, 0f);
                Global.getCombatEngine().addFloatingText(new Vector2f(0f, -75f), String.format("战场规模: %d", battleSize - 200), 50f, Color.yellow, null, 1f, 0f);
                if (progress > 0.0 && System.currentTimeMillis() - epoch >= 1000 * 60 * 2) {
                    ...
                    Global.getCombatEngine().addFloatingText(new Vector2f(0f, -125f), String.format("剩余时间: %d 分钟", Math.round(eta)), 50f, Color.yellow, null, 1f, 0f);
                }
            ...
            if (System.currentTimeMillis() - epochInfo >= 1000 * 10) {
                ...
                log.info(String.format("基准测试统计数据 [%s] (进度: %d%%):", new SimpleDateFormat("HH:mm:ss").format(new Date()), (int) (progress * 100.0)));
                log.info(String.format("  平均帧率: %.1f", fpsAvg));
                log.info(String.format("  最低帧率: %.1f", minimumFPS));
                log.info(String.format("  帧时间波动: %.2fms", stdevMs));
                log.info(String.format("  平均帧时间波动: %.2fms", variancesAvg));
                log.info(String.format("  平均内存占用: %.2fGB", meanMem));
                log.info(String.format("  最低内存占用: %.2fGB", minMem));
                log.info(String.format("  游戏速度: %.1f%%", gameSpeed * 100.0));
                log.info(String.format("  战场规模: %d", battleSize - 200));
