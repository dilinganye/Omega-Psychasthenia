...
//        var enableVersionChecker = LunaSettings.getBoolean("lunalib", "luna_enableVC")
    ...
    override fun init(engine: CombatEngineAPI?) {
        ...
        tip = font.createText(getString(1), Misc.getHighlightColor(), 15f)
        tip!!.append(
            getString(2).replaceToken("settingsKeybind", settingsKeybind),
            Misc.getBasePlayerColor()
        )
        tip!!.maxWidth = 400f

        settingsButtonText = font.createText(getString(3), Misc.getBasePlayerColor(), 15f)

        versionButtonText = font.createText(getString(4), Misc.getBasePlayerColor(), 15f)
    }
    ...
    fun getScreenPanel(): UIPanelAPI {
        ...
        val methods = titlescreen::class.java.methods as Array<Any>
    ...
    fun addModSettingsButton() {
        ...
        if (mouseX in x..x + buttonWidth && mouseY in y..y + buttonHeight) {
            ...
            if (cooldown < 1 && Mouse.isButtonDown(0) && !LunaSettingsUIMainPanel.panelOpen && !LunaVersionUIPanel.panelOpen) {
                ...
                try {
                    ...
                    settingsPanel = Global.getSettings().createCustom(
                        Global.getSettings().screenWidth * 0.8f,
                        Global.getSettings().screenHeight * 0.8f,
                        settingsPlugin
                    )
                    settingsPlugin!!.initFromScript(settingsPanel!!)
                    settingsPanel!!.position.inTL(
                        Global.getSettings().screenWidth * 0.1f,
                        Global.getSettings().screenHeight * 0.1f
                    )
        ...
        settingsButtonText!!.draw(
            (x + buttonWidth / 2) - settingsButtonText!!.width / 2f,
            (y + buttonHeight / 2) + settingsButtonText!!.height / 2f
        )
    }
    ...
    fun addVersionButton() {
        ...
        if (mouseX in x..x + buttonWidth && mouseY in y..y + buttonHeight) {
            ...
            if (cooldown < 1 && Mouse.isButtonDown(0) && !LunaSettingsUIMainPanel.panelOpen && !LunaVersionUIPanel.panelOpen) {
                ...
                try {
                    ...
                    versionPanel = Global.getSettings().createCustom(
                        Global.getSettings().screenWidth * 0.8f,
                        Global.getSettings().screenHeight * 0.8f,
                        versionPlugin
                    )
                    versionPlugin!!.initFromScript(versionPanel!!)
                    versionPanel!!.position.inTL(
                        Global.getSettings().screenWidth * 0.1f,
                        Global.getSettings().screenHeight * 0.1f
                    )
        ...
        versionButtonText!!.text = getString(4)
        if (count > 0) {
            versionButtonText!!.text = "       " + getString(4)
            versionButtonText!!.append("\n")
            versionButtonText!!.appendIndented(getString(5).replaceToken("count", count.toString()), Misc.getHighlightColor(), 0)
        }
        versionButtonText!!.draw(
            (x + buttonWidth / 2) - versionButtonText!!.width / 2f,
            (y + buttonHeight / 2) + versionButtonText!!.height / 2f
        )
    }
            ...
            if (true) {
                ...
                try {
                    ...
                    try {
                        ...
                        val getNameMethod = MethodHandles.lookup()
                            .findVirtual(fieldClass, "getName", MethodType.methodType(String::class.java))
                ...
                // 以下Else代码不会被执行
//                settingsKeybind = Keyboard.getKeyName(LunaSettings.getInt("lunalib", "luna_SettingsKeybind_new")!!)
//
//                val preText : String
//                if (Global.getSettings().modManager.isModEnabled("nexerelin"))
//                    preText = getString(6).replaceToken("settingsKeybind", settingsKeybind)
//                else
//                    preText = getString(7).replaceToken("settingsKeybind", settingsKeybind)
//                tip!!.draw(100f, 100f);
//                tip!!.text = getString(1)
//                tip!!.append(preText, Misc.getBasePlayerColor())
            }
