...
class LunaUIColorPicker(
    var value: Color?,
    var hasParagraph: Boolean,
    width: Float,
    height: Float,
    key: Any,
    group: String,
    ...
    override fun positionChanged(position: PositionAPI) {
        ...
        if (hueSlider == null && lunaElement != null && value != null) {
            ...
            val pan = lunaElement!!.createUIElement(width, height, false)
            ...
            textField!!.onUpdate {
                if (textField!!.isSelected()) {
                    try {
                        val color = Color.decode(textField!!.paragraph!!.text.replace("_", ""))
                        ...
                        val list = listOf(hueSlider, satSlider, brightSlider)
                        for (entry in list.indices) {
                            val min = centerX - width / 2 + width / 20
                            val max = centerX + width / 2 - width / 20

                            var level = (Color.RGBtoHSB(color.red, color.green, color.blue, null)
                                .get(entry) - 0f) / (1f - 0f)
                            level -= 0.5f
                            val scale = max - min
            ...
            satPara!!.position.inTL(
                (width / 2 - satPara!!.computeTextWidth(satPara!!.text) / 2),
                height * 0.31f + height * 0.23f + 8
            )

            brightPara = pan.addPara(
                "亮度",
                1f,
                Misc.getBasePlayerColor(),
                Misc.getBasePlayerColor()
            )
            brightPara!!.position.inTL(
                (width / 2 - brightPara!!.computeTextWidth(brightPara!!.text) / 2),
                height * 0.31f + height * 0.46f + 12
            )
