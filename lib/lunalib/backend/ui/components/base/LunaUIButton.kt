...
class LunaUIButton(
    ...
    override fun positionChanged(position: PositionAPI) {
        ...
        if (buttonText != null) {
            if (regularButton) {
                buttonText!!.position.inTL(
                    width / 2 - buttonText!!.computeTextWidth(buttonText!!.text) / 2,
                    height / 2 - buttonText!!.computeTextHeight(buttonText!!.text) / 2
                )
            }
        }
    }
    ...
    override fun advance(amount: Float) {
        ...
        if (buttonText != null) {
            if (regularButton) {
                if (value) {
                    buttonText!!.text = Global.getSettings().getString("LunaLib_Core_backend", "LunaUIButton_1")
                } else {
                    buttonText!!.text = Global.getSettings().getString("LunaLib_Core_backend", "LunaUIButton_2")
                }
                buttonText!!.position.inTL(
                    width / 2 - buttonText!!.computeTextWidth(buttonText!!.text) / 2,
                    height / 2 - buttonText!!.computeTextHeight(buttonText!!.text) / 2
