...
class LunaDebugUISnippetsPanel : LunaDebugUIInterface {
    ...
    companion object {
        ...
        var filters = mutableListOf(
            "全部",
            LunaSnippet.SnippetTags.Cheat.toString(),
            LunaSnippet.SnippetTags.Debug.toString(),
    ...
    override fun init(parentPanel: CustomPanelAPI, parentClass: LunaDebugUIMainPanel, panel: CustomPanelAPI) {
        ...
        filters = mutableListOf(
            "全部", LunaSnippet.SnippetTags.Cheat.toString(), LunaSnippet.SnippetTags.Debug.toString(),
            LunaSnippet.SnippetTags.Cargo.toString(),
            LunaSnippet.SnippetTags.Entity.toString(),
            LunaSnippet.SnippetTags.Player.toString(),
            LunaSnippet.SnippetTags.Faction.toString()
        ...
        for (filter in filters) {
            var button = LunaUIButton(false, false, 250f, 30f, filter, "Debug", panel!!, tagScroller!!).apply {
                ...
                if (tooltip != "") tagScroller!!.addTooltipToPrevious(
                    TooltipHelper(tooltip, 300f),
    ...
    fun createList() {
        ...
        for (snippet in snippets) {
            ...
            var descriptionElement =
                cardPanel.lunaElement!!.createUIElement(subpanel!!.position.width * 0.6f - 20, requiredSpacing, false)
            ...
            var descriptionText = categories + "\n\n" + snippet.getDescription()
            ...
            var interactbleElement =
                cardPanel.lunaElement!!.createUIElement(subpanel!!.position.width * 0.4f - 20, requiredSpacing, false)
            ...
            var executeButton =
                LunaUIButton(true, false, 250f, 30f, "", "Debug", cardPanel.lunaElement!!, interactbleElement!!)
            executeButton.buttonText!!.text = "执行"
            executeButton.buttonText!!.position.inTL(
                executeButton.position!!.width / 2 - executeButton.buttonText!!.computeTextWidth(
                    executeButton.buttonText!!.text
                ) / 2,
                executeButton!!.position!!.height / 2 - executeButton.buttonText!!.computeTextHeight(executeButton.buttonText!!.text) / 2
            )
            executeButton.position!!.inTL(
                interactbleElement.position!!.width / 2 - executeButton.position!!.width / 2,
                20f
            )
            ...
            executeButton.onClick {
                ...
                var output =
                    LunaUIPlaceholder(true, w - 310, h * 0.30f, "empty", "none", outputPanel!!, outputElement!!).apply {
                        backgroundAlpha = 0.75f
                        borderAlpha = 0.75f
                    }
                ...
                try {
                    ...
                    Global.getLogger(this::class.java)
                        .debug("Lunalib: Failed to execute snippet \"${snippet.getName()}\"")
                    outputScroller.addPara("Error: 脚本执行失败！", 0f)
                }
                output.lunaElement!!.addUIElement(outputScroller)
    ...
    fun searchForSnippets() {
        ...
        for (snippet in LunaDebug.snippets.sortedBy { snippet -> snippet.name }) {
            try {
                ...
                if (activeFilter != "全部" && !snippet.getTags().contains(activeFilter)) continue

                var result = FuzzySearch.extractOne(
                    text,
                    listOf(snippet.name.lowercase(), mod.id.lowercase(), mod.name.lowercase())
                )
                var failed = false
                if (!snippet.getName().lowercase().contains(text) && !mod.id.lowercase()
                        .contains(text) && !mod.name.lowercase().contains(text)
                ) failed = true
