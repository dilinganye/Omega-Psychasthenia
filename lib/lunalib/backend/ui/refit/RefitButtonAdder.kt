...
/**
 * 这个事件类负责Hack舰船装配框架，然后被用于显示 Random Assert of Things 的那些特殊深渊插件。
 */
class RefitButtonAdder : EveryFrameScript {
    ...
    override fun advance(amount: Float) {
        ...
        if (base != null) {
            ...
            var openButton = openButtonElement.addLunaSpriteElement(
                "graphics/ui/lunaRefitButtonMain.png",
            ...
            openButtonElement.addTooltipToPrevious(object : TooltipMakerAPI.TooltipCreator {
                ...
                override fun createTooltip(tooltip: TooltipMakerAPI?, expanded: Boolean, tooltipParam: Any?) {
                    tooltip!!.addPara(getString(1), 0f)
                }
            ...
            var openPara = openButtonElement.addTitle(getString(2).replaceToken("sizeNum", size.toString()), Misc.getBasePlayerColor().setAlpha(0))
            ...
            openButton.advance {
                if (member != null && variant != null) {
                    ...
                    if (buttons != lastCount) {
                        lastCount = buttons
                        openPara.text = getString(2).replaceToken("sizeNum", buttons.toString())
                        openPara.position.inTL(
                            openButtonElement.position.width / 2 - openPara.computeTextWidth(openPara.text) / 2 + 5,
                            1f
                        )
                    }
    ...
    fun recreateBackgroundPanel() {
        ...
        var closeButton = closeButtonElement.addLunaSpriteElement(
            "graphics/ui/lunaRefitButtonMain.png",
            LunaSpriteElement.ScalingTypes.STRETCH_SPRITE,
            mainPanel!!.position.width,
            20f
        ).apply {
            getSprite().color = Misc.getDarkPlayerColor().setAlpha(255)
            innerElement.bringComponentToTop(innerElement.prev)
        ...
        var closePara = closeButtonElement.addTitle(getString(3), Misc.getBasePlayerColor())
        closePara.position.inTL(
            closeButtonElement.position.width / 2 - closePara.computeTextWidth(closePara.text) / 2,
            1f
        )
        closeButtonElement.position.inTL(0f, 0f)


        var buttonElement = backgroundPanel!!.createUIElement(
            backgroundPanel!!.position.width,
            backgroundPanel!!.position.height - closeButton.height,
            true
        )
        ...
        for (button in LunaRefitManager.buttons.sortedBy { it.getOrder(member, variant) }) {
            ...
            var sprite = buttonElement.addLunaSpriteElement(
                button.getIconName(member, variant),
                LunaSpriteElement.ScalingTypes.STRETCH_SPRITE,
                0f,
                0f
            ).apply {
            ...
            var uiButton = buttonElement.addLunaElement(225f, 40f).apply {
                ...
                onClick {
                    ...
                    if (button.hasPanel(member, variant, market)) {
                        ...
                        activePanel!!.position.inTL(
                            Global.getSettings().screenWidth / 2 - width / 2,
                            Global.getSettings().screenHeight / 2 - height / 2
                        )
    ...
    private fun getString(id : Int) : String {
        val className = "RefitButtonAdder"
