...
class LunaSettingsUIModsPanel(var newGame: Boolean) : CustomUIPanelPlugin {
    ...
    fun init(parentPanel: CustomPanelAPI, panel: CustomPanelAPI) {
        ...
        saveButton =
            LunaUIButton(false, false, width - 15, 30f, "Test", "SettingGroup", panel!!, panelElement!!).apply {
                this.buttonText!!.text = getString(1)
                this.buttonText!!.setHighlight(getString(1))
                ...
                this.uiElement.addTooltipToPrevious(
                    TooltipHelper(
                        getString(2),
                        300f,
                        getString(2, 1), getString(2, 2), getString(2, 3)
        ...
        resetButton =
            LunaUIButton(false, false, width - 15, 30f, "Test", "SettingGroup", panel!!, panelElement!!).apply {
                this.buttonText!!.text = getString(3)
                this.buttonText!!.position.inTL(
                    this.buttonText!!.position.width / 2 - this.buttonText!!.computeTextWidth(
                        this.buttonText!!.text
                    ) / 2,
        ...
        resetButton!!.onClick {
            ...
            for (element in LunaSettingsUISettingsPanel.addedElements) {
                ...
                if (element is LunaUIKeybindButton) {
                    ...
                    if (value == 0) {
                        element.button!!.buttonText!!.text = "键位: 无"
                        element.button!!.buttonText!!.setHighlight("无")
                    } else {
                        element.button!!.buttonText!!.text = "键位: ${Keyboard.getKeyName(value)}"
                        element.button!!.buttonText!!.setHighlight(Keyboard.getKeyName(value))
                    }
                    element.button!!.buttonText!!.position.inTL(
                        element.width / 2 - element.button!!.buttonText!!.computeTextWidth(
                            element.button!!.buttonText!!.text
                        ) / 2,
                        element.height / 2 - element.button!!.buttonText!!.computeTextHeight(element.button!!.buttonText!!.text) / 2
                    )
        ...
        var aboutButton =
            LunaUIButton(false, false, width - 15, 30f, "Test", "ModsButton", panel!!, panelElement!!).apply {
                this.buttonText!!.text = getString(5)
                this.buttonText!!.position.inTL(
                    this.buttonText!!.position.width / 2 - this.buttonText!!.computeTextWidth(
                        this.buttonText!!.text
                    ) / 2,
        ...
        var para =
            pan.addPara(getString(6), 0f, Misc.getBasePlayerColor(), Misc.getBasePlayerColor())
        para.position.inTL(
        ...
        searchField.run {
            this.uiElement.addTooltipToPrevious(
                TooltipHelper(getString(7), 300f),
                TooltipMakerAPI.TooltipLocation.RIGHT
            )
        }
    ...
    private fun createModsList() {
        ...
        val mods: List<ModSpecAPI> =
            Global.getSettings().modManager.enabledModsCopy.filter { modsWithData.contains(it.id) }
        ...
        for (mod in mods) {
            ...
            if (currentSearchText != "" && !mod.name.lowercase()
                    .contains(currentSearchText.lowercase()) && !mod.id.lowercase()
                    .contains(currentSearchText.lowercase())
            ) failed = true
            ...
            var cardPanel =
                LunaUIPlaceholder(true, width - 15, 60f, mod, "ModsButton", subpanel!!, subpanelElement!!).apply {

                    var text = getString(8).replaceToken("modName", mod.name).replaceToken("modAuthorName", mod.author).replaceToken("modVersion", mod.version).replaceToken("modDescription", mod.desc)

                    this.uiElement.addTooltipToPrevious(
                        TooltipHelper(text, 400f, mod.name),
                        TooltipMakerAPI.TooltipLocation.RIGHT
                    )
            ...
            var sprite =
                LunaUISprite(icon, 40f, 40f, 40f, 40f, 40f, 40f, "", "Group", cardPanel.lunaElement!!, spriteElement!!)
        ...
        if (saving) {
            ...
            if (saveDelay < 1) {
                ...
                for (mod in changedMods) {
                    val data =
                        JSONUtils.loadCommonJSON("LunaSettings/${mod}.json", "data/config/LunaSettingsDefault.default");
    ...
    private fun getString(id : Int) : String {
        val className = "LunaSettingsUIModsPanel"
