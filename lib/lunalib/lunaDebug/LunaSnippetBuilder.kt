...
class SnippetBuilder(
    private var cardPanel: LunaUIPlaceholder,
    private var panel: CustomPanelAPI,
    private var parameterCard: TooltipMakerAPI
) {
    ...
    fun addStringParameter(name: String, key: String) {
        ...
        var pan =
            textField.lunaElement!!.createUIElement(textField.position!!.width, textField.position!!.height, false)
        ...
        para.position.inTL(
            textField.position!!.width / 2 - para.computeTextWidth(para.text) / 2,
            textField.position!!.height / 2 - para.computeTextHeight(para.text) / 2
        )
    ...
    private fun <T : Number> addNumberParameter(name: String, key: String, defaultValue: T, minValue: T, maxValue: T) {
        var textFieldAndSlider = LunaUITextFieldWithSlider(
            defaultValue,
            minValue.toFloat(),
            maxValue.toFloat(),
            250f,
        ...
        var pan = textFieldAndSlider.lunaElement!!.createUIElement(
            textFieldAndSlider.position!!.width,
            textFieldAndSlider.position!!.height,
            false
        )
        ...
        var para = pan.addPara(name, 0f, Misc.getBasePlayerColor(), Misc.getBasePlayerColor())
        para.position.inTL(
            textFieldAndSlider.position!!.width - para.computeTextWidth(para.text) - 5,
            textFieldAndSlider.textField!!.position!!.height / 2 - para.computeTextHeight(para.text) / 2
        )
