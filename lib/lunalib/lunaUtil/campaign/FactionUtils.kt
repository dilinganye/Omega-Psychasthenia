...
object FactionUtils {
    ...
    fun existsInSector(FactionId: String): Boolean {
        var found = false
        Global.getSector().economy.marketsCopy.forEach {
    ...
    fun getKnownShipSpecs(FactionId: String): List<ShipHullSpecAPI> =
        Global.getSettings().allShipHullSpecs.filter {
            Global.getSector().getFaction(FactionId).knownShips.contains(it.hullId)
        }
    ...
    fun getKnownHullmodSpecs(FactionId: String): List<HullModSpecAPI> =
        Global.getSettings().allHullModSpecs.filter {
            Global.getSector().getFaction(FactionId).knownHullMods.contains(it.id)
        }
    ...
    fun getKnownWeaponSpecs(FactionId: String): List<WeaponSpecAPI> =
        Global.getSettings().allWeaponSpecs.filter {
            Global.getSector().getFaction(FactionId).knownWeapons.contains(it.weaponId)
        }
    ...
    fun getKnownFighterSpecs(FactionId: String): List<FighterWingSpecAPI> =
        Global.getSettings().allFighterWingSpecs.filter {
            Global.getSector().getFaction(FactionId).knownFighters.contains(it.id)
        }
