...
class LunaCampaignRenderer : EveryFrameScript {
    ...
    @Transient
    private var transientRenderers: ArrayList<LunaCampaignRenderingPlugin>? = ArrayList()
    ...
    override fun advance(amount: Float) {
        ...
        if (currentLocation != null) {
            if (currentLocation.customEntities.none { it.customEntitySpec?.id == "luna_campaign_renderer" }) {
                var entity = currentLocation.addCustomEntity(
                    "luna_campaign_renderer_${Misc.genUID()}",
                    "",
        ...
        if (interval.intervalElapsed()) {
            ...
            for (location in locations) {
                if (location.customEntities.none { it.customEntitySpec?.id == "luna_campaign_renderer" }) {
                    var entity = location.addCustomEntity(
                        "luna_campaign_renderer_${Misc.genUID()}",
                        "",
                        "luna_campaign_renderer",
                        Factions.NEUTRAL
