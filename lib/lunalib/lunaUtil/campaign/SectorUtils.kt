...
object SectorUtils {
    ...
    fun getPlanetsWithCondition(conditionID: String): List<PlanetAPI> =
        Global.getSector().starSystems.flatMap { system ->
     ...
     */
    @JvmStatic
    fun getSystemsWithTag(tag: String): List<StarSystemAPI> =
        Global.getSector().starSystems.filter { it.tags.contains(tag) }
