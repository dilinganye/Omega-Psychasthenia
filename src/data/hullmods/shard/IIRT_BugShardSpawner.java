...
import static data.hullmods.shard.PTSD_BaseShard_Util.BugvariantData;

public class IIRT_BugShardSpawner extends BaseHullMod {

	public static Color JITTER_COLOR = new Color(200, 144, 255, 50);
	...
	public void advanceInCombat(ShipAPI ship, float amount) {
		...
		while (addedWeight < splitWeight) {
			...
			if (cruiser) {
				PTSD_BaseShard_Util.ShardTypeVariants variants = BugvariantData.get(HullSize.CRUISER);
			...
			if (destroyer && variant == null) {
				PTSD_BaseShard_Util.ShardTypeVariants variants = BugvariantData.get(HullSize.DESTROYER);
				WeightedRandomPicker<String> variantPicker = variants.get(type);
				variant = variantPicker.pick();
				if (variant != null) {
					weight = 2f;
			...
			if (frigate && variant == null) {
				PTSD_BaseShard_Util.ShardTypeVariants variants = BugvariantData.get(HullSize.FRIGATE);
				WeightedRandomPicker<String> variantPicker = variants.get(type);
				variant = variantPicker.pick();
				if (variant != null) {
					weight = 1f;
			...
			if (variant == null) {
				PTSD_BaseShard_Util.ShardTypeVariants variants = BugvariantData.get(HullSize.FIGHTER);
				WeightedRandomPicker<String> variantPicker = variants.get(type);
				variant = variantPicker.pick();
				if (variant != null) {
					fighters++;
