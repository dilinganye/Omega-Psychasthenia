...
public class IIRT_ColdShardSpawner extends BaseHullMod {

	public static Color JITTER_COLOR = new Color(176, 255, 248, 50);
	...
	public static Map<HullSize, ShardTypeVariants> variantData = new HashMap<>();

	static {
		ShardTypeVariants fighters = new ShardTypeVariants();
		variantData.put(HullSize.FIGHTER, fighters);
		fighters.get(ShardType.GENERAL).add("IIRT_Omega_Nihility_attack_wing", 10f);
		fighters.get(ShardType.MISSILE).add("IIRT_Omega_Nihility_missile_wing", 15f);
		fighters.get(ShardType.POINT_DEFENSE).add("IIRT_Omega_Nihility_shock_wing", 10f);
		fighters.get(ShardType.ANTI_SHIELD).add("IIRT_Omega_Nihility_shieldbreaker_wing", 10f);

		ShardTypeVariants small = new ShardTypeVariants();
		variantData.put(HullSize.FRIGATE, small);

		small.get(ShardType.GENERAL).add("IIRT_Omega_Heatdeath_Normal", 5f);
		small.get(ShardType.GENERAL).add("IIRT_Omega_Heatdeath_Normal_Plus", 5f);

		small.get(ShardType.POINT_DEFENSE).add("IIRT_Omega_Heatdeath_Support", 10f);
		small.get(ShardType.MISSILE).add("IIRT_Omega_Heatdeath_Missile", 10f);
		small.get(ShardType.ANTI_ARMOR).add("IIRT_Omega_Heatdeath_AntiArmor", 10f);
		small.get(ShardType.ANTI_SHIELD).add("IIRT_Omega_Heatdeath_Missile", 10f);

		small.get(ShardType.GENERAL).add("IIRT_Omega_Nihility_attack_wing", 1f);
		small.get(ShardType.MISSILE).add("IIRT_Omega_Nihility_missile_wing", 1f);
		small.get(ShardType.POINT_DEFENSE).add("IIRT_Omega_Nihility_shock_wing", 1f);
		small.get(ShardType.ANTI_SHIELD).add("IIRT_Omega_Nihility_shieldbreaker_wing", 1f);

		//They are not been made yet, so i just put it here.)
		ShardTypeVariants medium = new ShardTypeVariants();
		variantData.put(HullSize.DESTROYER, medium);

		medium.get(ShardType.GENERAL).add("IIRT_Omega_Singularity_Attack", 6f);
		medium.get(ShardType.GENERAL).add("IIRT_Omega_Singularity_Attack_Plus", 4f);
		medium.get(ShardType.ANTI_ARMOR).add("IIRT_Omega_Singularity_AntiArmor", 10f);
		medium.get(ShardType.ANTI_SHIELD).add("IIRT_Omega_Singularity_AntiShield", 10f);
		medium.get(ShardType.POINT_DEFENSE).add("IIRT_Omega_Singularity_Support", 10f);
		medium.get(ShardType.MISSILE).add("IIRT_Omega_Singularity_Missile", 10f);
		...
		large.get(ShardType.ANTI_ARMOR).add("IIRT_Omega_Firewall_Only_2", 10f);
		large.get(ShardType.ANTI_SHIELD).add("IIRT_Omega_Firewall_Only", 10f);
		large.get(ShardType.POINT_DEFENSE).add("IIRT_Omega_Antitrack_Only_3", 10f);
	...
	public void advanceInCombat(ShipAPI ship, float amount) {
		...
		WeightedRandomPicker<Float> spawnAngles = new WeightedRandomPicker<>();
	...
	public WeightedRandomPicker<ShardType> getTypePickerBasedOnLocalConditions(ShipAPI ship) {
		...
		WeightedRandomPicker<ShardType> picker = new WeightedRandomPicker<>();
	...
	public static void assignColdShardSpawnToFleet(com.fs.starfarer.api.campaign.CampaignFleetAPI fleet, int spawnPoints) {
		...
		try {
			...
			while (remaining > 0 && safety < 200) {
				...
				HullSize size;
				int cost;
				if (remaining > 80) { size = HullSize.CRUISER; cost = 100; }
				else if (remaining > 30) { size = HullSize.DESTROYER; cost = 50; }
				else if (remaining >= 10) { size = HullSize.FRIGATE; cost = 20; }
				else { size = HullSize.FIGHTER; cost = 5; }

				String variant = pickVariantForSize(size);
				if (variant == null) {
					// try fallbacks
					if (size != HullSize.FRIGATE) variant = pickVariantForSize(HullSize.FRIGATE);
					if (variant == null) variant = pickVariantForSize(HullSize.FIGHTER);
				}
			...
			Global.getLogger(IIRT_ColdShardSpawner.class).info("assignShardSpawnToFleet failed: " + t.getMessage());
		}
	}

	private static String pickVariantForSize(HullSize size) {
	...
	protected ShardFadeInPlugin createShipFadeInPlugin(final String variantId, final ShipAPI source, final float delay, final float fadeInTime, final float angle) {
