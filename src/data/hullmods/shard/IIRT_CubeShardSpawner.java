...
import static data.hullmods.shard.PTSD_BaseShard_Util.CubevariantData;

public class IIRT_CubeShardSpawner extends BaseHullMod {

	public static Color JITTER_COLOR = new Color(189, 143, 255, 68);
	...
	public void advanceInCombat(ShipAPI ship, float amount) {
		...
		while (addedWeight < splitWeight) {
			...
			if (cruiser) {
				PTSD_BaseShard_Util.ShardTypeVariants variants = CubevariantData.get(ShipAPI.HullSize.CRUISER);
			...
			if (destroyer && variant == null) {
				PTSD_BaseShard_Util.ShardTypeVariants variants = CubevariantData.get(HullSize.DESTROYER);
				WeightedRandomPicker<String> variantPicker = variants.get(type);
				variant = variantPicker.pick();
				if (variant != null) {
					weight = 2f;
			...
			if (frigate && variant == null) {
				PTSD_BaseShard_Util.ShardTypeVariants variants = CubevariantData.get(ShipAPI.HullSize.FRIGATE);
				WeightedRandomPicker<String> variantPicker = variants.get(type);
				variant = variantPicker.pick();
				if (variant != null) {
					weight = 1f;
			...
			if (variant == null) {
				PTSD_BaseShard_Util.ShardTypeVariants variants = CubevariantData.get(ShipAPI.HullSize.FIGHTER);
				WeightedRandomPicker<String> variantPicker = variants.get(type);
				variant = variantPicker.pick();
				if (variant != null) {
					fighters++;
