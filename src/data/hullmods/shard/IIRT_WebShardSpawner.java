...
public class IIRT_WebShardSpawner extends BaseHullMod {

	public static Color JITTER_COLOR = new Color(115, 100, 255, 50);
	...
	public void advanceInCombat(ShipAPI ship, float amount) {
		...
		float capitalProb = 0f;
		float capitalProbMult = 0f;
		float maxcapitals = 0f;
		...
		if (ship.isCapital()) {
			...
			frigateProbMult = 1f;
			maxFrigates = 4f;
			if(ship.getHullSpec().getBaseHullId().equals("PTSD_Omega_MacroVirus")){
				splitWeight = 20f;
				capitalProb = 1f;
				capitalProbMult = 1f;
				maxcapitals = 2f;
		...
		WeightedRandomPicker<PTSD_BaseShard_Util.ShardType> typePicker = getTypePickerBasedOnLocalConditions(ship);
		...
		while (addedWeight < splitWeight) {
			PTSD_BaseShard_Util.ShardType type = typePicker.pick();

			float rem = splitWeight - addedWeight;
			boolean capital = 0 < capitalProb && capitals <= maxcapitals;
			...
			if (capital) {
				PTSD_BaseShard_Util.ShardTypeVariants variants = PTSD_BaseShard_Util.WebvariantData.get(ShipAPI.HullSize.CAPITAL_SHIP);
				WeightedRandomPicker<String> variantPicker = variants.get(type);
				variant = variantPicker.pick();
				if (variant != null) {
					weight = 8f;
					capitals++;
					capitalProb *= capitalProbMult;
				}
			}
			if (cruiser) {
				PTSD_BaseShard_Util.ShardTypeVariants variants = PTSD_BaseShard_Util.WebvariantData.get(ShipAPI.HullSize.CRUISER);
				WeightedRandomPicker<String> variantPicker = variants.get(type);
				variant = variantPicker.pick();
				if (variant != null) {
					weight = 4f;
			...
			if (destroyer && variant == null) {
				PTSD_BaseShard_Util.ShardTypeVariants variants = PTSD_BaseShard_Util.WebvariantData.get(ShipAPI.HullSize.DESTROYER);
				WeightedRandomPicker<String> variantPicker = variants.get(type);
				variant = variantPicker.pick();
				if (variant != null) {
					weight = 2f;
			...
			if (frigate && variant == null) {
				PTSD_BaseShard_Util.ShardTypeVariants variants = PTSD_BaseShard_Util.WebvariantData.get(ShipAPI.HullSize.FRIGATE);
				WeightedRandomPicker<String> variantPicker = variants.get(type);
				variant = variantPicker.pick();
				if (variant != null) {
					weight = 1f;
			...
			if (variant == null) {
				PTSD_BaseShard_Util.ShardTypeVariants variants = PTSD_BaseShard_Util.WebvariantData.get(ShipAPI.HullSize.FIGHTER);
				WeightedRandomPicker<String> variantPicker = variants.get(type);
				variant = variantPicker.pick();
				if (variant != null) {
					fighters++;
	...
	public WeightedRandomPicker<PTSD_BaseShard_Util.ShardType> getTypePickerBasedOnLocalConditions(ShipAPI ship) {
		CombatEngineAPI engine = Global.getCombatEngine();
		...
		WeightedRandomPicker<PTSD_BaseShard_Util.ShardType> picker = new WeightedRandomPicker<>();
		...
		picker.add(PTSD_BaseShard_Util.ShardType.GENERAL, 0.0f + (1f - friends) * 0.4f);

		float unlikelyWeight = 0f;
		float unlikelyThreshold = 0.2f;

		if (antiFighter < unlikelyThreshold) antiFighter = unlikelyWeight;
		picker.add(PTSD_BaseShard_Util.ShardType.POINT_DEFENSE, antiFighter);

		if (antiShield < unlikelyThreshold) antiShield = unlikelyWeight;
		picker.add(PTSD_BaseShard_Util.ShardType.ANTI_SHIELD, antiShield);

		if (antiArmor < unlikelyThreshold) antiArmor = unlikelyWeight;
		picker.add(PTSD_BaseShard_Util.ShardType.ANTI_ARMOR, antiArmor);

		if (missile < unlikelyThreshold) missile = unlikelyWeight;
		picker.add(PTSD_BaseShard_Util.ShardType.MISSILE, missile);
