...
public class IIRT_SD_Yisic_Lab extends BaseIndustry implements RouteFleetSpawner, FleetEventListener {

	//当不在ID为”IIRT“的势力控制时隐藏此建筑
	@Override
	public boolean isHidden() {
		return !market.getFactionId().contentEquals("IIRT");
	}
	...
	public void apply() {
		...
		supply(Commodities.FUEL, size); //提供燃料
		supply(Commodities.SUPPLIES, size + 2); //提供补给
		supply(Commodities.HAND_WEAPONS, size); //提供武器(物品栏的)
		supply(Commodities.MARINES, size);  //提供四等人
		supply(Commodities.ORE, size - 1);  //提供铁矿石
		supply(Commodities.ORGANICS, size - 1);  //提供有机物
		supply(Commodities.VOLATILES, size - 4);    //提供气体
		supply(Commodities.FOOD, size - 3); //提供食物

		Pair<String, Integer> deficit = getMaxDeficit(Commodities.HAND_WEAPONS);//获取最大赤字
		applyDeficitToProduction(1, deficit, Commodities.MARINES);//将赤字应用于生产
		...
		if (isFunctional()) {   //如果行业是功能性的
			market.getStats().getDynamic().getMod(Stats.COMBAT_FLEET_SIZE_MULT).modifyMult( //调整舰队大小倍数
					getModId(), 1f + 1f, "以西卡研究所分部 (" + getNameForModifier() + ")");
		} else {
			unapply();
		}
	}
	...
	public void unapply() {
		super.unapply();
		MemoryAPI memory = market.getMemoryWithoutUpdate(); //获取市场内存
		Misc.setFlagWithReason(memory, MemFlags.MARKET_PATROL, getModId(), false, -1);  //设置标志
		Misc.setFlagWithReason(memory, MemFlags.MARKET_MILITARY, getModId(), false, -1);    //设置标志
	...
	public CampaignFleetAPI spawnFleet(RouteData route) {
		...
		FleetParamsV3 params = new FleetParamsV3(market, null, "IIRT", route.getQualityOverride(), fleetType, combat, freighter, 0f, 0f, 0f, 0f, 0f);
