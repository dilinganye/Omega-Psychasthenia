...
import static data.scripts.IIRT_Omega_ModPlugin.final_invasion_max_strength;
import static data.scripts.IIRT_Omega_ModPlugin.isInvasionEnabled;
...
public class IIRT_Omega_ReserveAttacker implements EveryFrameScript {
    ...
    public boolean isDone() {
        ...
        if (Global.getSector().getPersistentData().containsKey(PTSDCrisisState.PERSISTENT_KEY)) return true;
        Object stage = Global.getSector().getMemoryWithoutUpdate().get(IIRT_Omega_Invasion.stage_id);
        return stage == IIRT_Omega_Invasion.STAGE.END || "END".equals(stage)
                || Global.getSector().getMemoryWithoutUpdate().getBoolean("$IIRT_omega_Invasion_End");
    }
    ...
    public void advance(float amount) {
        if (Global.getSector() == null || sector == null || !isInvasionEnabled()) return;
        if (Global.getSector().getPersistentData().containsKey(PTSDCrisisState.PERSISTENT_KEY)) return;
