...
public class IIRT_SD_Market_Army extends BaseSubmarketPlugin {
	...
	public void updateCargoPrePlayerInteraction() {
		...
		if (okToUpdateShipsAndWeapons()) {  //若可以更新，则继续进行。否则返回false
			...
			// 根据FactionId“IIRT_Army”获取”IIRT_Market“对象
			FactionAPI IIRT_Market_Army = null;
			List<FactionAPI> Factions = Global.getSector().getAllFactions();
			for (FactionAPI F : Factions) {
				if (F.getId().contentEquals("IIRT_Army")) {
					IIRT_Market_Army = F;
				}
			}

			// 添加战斗机和武器
			addFighters(fighterNum, fighterNum, 3, "IIRT_Army"); //最小数值, 最大数值, 最大堆叠(武器同时出现最大数), 阵营ID
			addWeapons(weapons, weapons + 2, 3, "IIRT_Army");
	...
	public boolean isIllegalOnSubmarket(CargoStackAPI stack, TransferAction action) {
		...
		if (action == TransferAction.PLAYER_BUY && !IIRT_Level.isAtWorst(RepLevel.FRIENDLY)) return true;
		return action == TransferAction.PLAYER_BUY && !IIRT_Level.isAtWorst(RepLevel.FRIENDLY);
	}
	...
	public String getIllegalTransferText(CargoStackAPI stack, TransferAction action) {
		...
		if (!IIRT_Level.isAtWorst(RepLevel.FRIENDLY)) {
	...
	public String getIllegalTransferText(FleetMemberAPI member, TransferAction action) {
		...
		if (!IIRT_Level.isAtWorst(RepLevel.FRIENDLY)) return "在[好感：友好]之前,你无法在此交易";
		return "你无法在此交易";
