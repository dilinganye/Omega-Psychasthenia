...
public class IIRT_SD_Market_Nor extends BaseSubmarketPlugin {
	...
	public void updateCargoPrePlayerInteraction() {
		...
		if (okToUpdateShipsAndWeapons()) {  //若可以更新，则继续进行。否则返回false
			...
			// 计算需要生成的武器数量
			int weapons = Math.max(6, market.getSize()) - 2;
			int fighterNum = Math.max(6, market.getSize()) - 5;
			int hullmods = Math.max(1, market.getSize() - 5);

			// 根据FactionId“IIRT_Nor”获取”IIRT_Market“对象
			FactionAPI IIRT_Market_Nor = null;
			List<FactionAPI> Factions = Global.getSector().getAllFactions();
			for (FactionAPI F : Factions) {
				if (F.getId().contentEquals("IIRT_Nor")) {
					IIRT_Market_Nor = F;
				}
			}

			// 添加战斗机和武器
			addFighters(fighterNum, fighterNum, 3, "IIRT_Nor"); //最小数值, 最大数值, 最大堆叠(武器同时出现最大数), 阵营ID
			addWeapons(weapons, weapons + 2, 3, "IIRT_Nor");
	...
	public boolean isIllegalOnSubmarket(CargoStackAPI stack, TransferAction action) {
		...
		if (action == TransferAction.PLAYER_BUY && !IIRT_Level.isAtWorst(RepLevel.NEUTRAL)) return true;
		return action == TransferAction.PLAYER_BUY && !IIRT_Level.isAtWorst(RepLevel.NEUTRAL);
	}
	...
	public String getIllegalTransferText(CargoStackAPI stack, TransferAction action) {
		...
		if (!IIRT_Level.isAtWorst(RepLevel.NEUTRAL)) {
	...
	public String getIllegalTransferText(FleetMemberAPI member, TransferAction action) {
		...
		if (!IIRT_Level.isAtWorst(RepLevel.NEUTRAL)) return "在[好感：中立]之前,你无法在此交易";
		return "你无法在此交易";
