...
public class IIRT_BombFighterStrikeStats extends BaseShipSystemScript implements DroneStrikeStatsAIInfoProvider {
	...
	public void convertDrones(ShipAPI ship, final ShipAPI target) {
		...
		for (ShipAPI drone : drones) {
			if (num < getNumToFire()) {
				...
				drone.setWing(null);
				drone.setExplosionFlashColorOverride(new Color(32, 0, 32, 255));
				...
				EmpArcEntityAPI arc = engine.spawnEmpArcVisual(ship.getLocation(), ship, missile.getLocation(), missile, thickness, new Color(240, 190, 240, 255), Color.white);
