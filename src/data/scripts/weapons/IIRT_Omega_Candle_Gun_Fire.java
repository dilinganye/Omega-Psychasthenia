...
public class IIRT_Omega_Candle_Gun_Fire implements EveryFrameWeaponEffectPlugin {
	...
	public void advance(float amount, CombatEngineAPI engine, WeaponAPI weapon) {
		...
		if (!weapon.isDisabled()) {
			...
			if (weapon.getAmmo() >= 10f) {
				moreThan5 = true;
			}
			if (weapon.getAmmo() < 10f) {
				moreThan5 = false;
			}
			if (firing && weapon.getAmmo() >= 10f) {
				weapon.setAmmo(0);
			}
			...
			if (!moreThan5) {
				...
				if (MagicRender.screenCheck(0.1f, weapon.getLocation())) {
					...
					engine.addSmoothParticle(muzzle, Speedo2, 30 * Kode, 2f, 0.15f, Color.white);
			...
			if (moreThan5) {
				...
				if (MagicRender.screenCheck(0.1f, weapon.getLocation())) {
					...
					engine.addHitParticle(muzzle, Speedo2, 20, 0.5f, 1, Color.blue);
					engine.addHitParticle(muzzle, Speedo2, 20, 1f, 0.3f, Color.red);
					engine.addSmoothParticle(muzzle, Speedo2, 30, 2f, 0.15f, Color.white);
					engine.addSmoothParticle(muzzle, Speedo2, 50, 2f, 0.1f, Color.white);

					easyRippleOut(muzzle, Speedo2, 50, 1, 0, 0.25f);
				}

				Vector2f SpeedoO = new Vector2f(((float)Vectorx) * 10, ((float)Vectory) * 10f);
				MagicRender.battlespace(Global.getSettings().getSprite("fx", "IIRT_Impact_Directional_01"), point, SpeedoO, new Vector2f(32, 64), new Vector2f(64, 96),
						//angle,
						weapon.getCurrAngle() + 90f,
						//+2f*(float)Math.random()+90f
						0, new Color(232, 238, 255, 100), true, 0, 0.1f, 0.25f);
    ...
    public void advance(float amount, CombatEngineAPI engine, WeaponAPI weapon) {
        ...
        if(moreThan5){
            ...
            MagicRender.battlespace(
                    Global.getSettings().getSprite("fx","IIRT_Impact_Directional_01"),
                    point,
                    Speedo2,
                    new Vector2f(32,64),
                    new Vector2f(64,96),
