...
public class IIRT_Omega_FighterStrikeStats_NOTUSED extends BaseShipSystemScript implements DroneStrikeStatsAIInfoProvider {
	...
	public void apply(MutableShipStatsAPI stats, String id, State state, float effectLevel) {
		...
		for (ShipAPI drone : getDrones(ship)) {
			...
			drone.setExplosionFlashColorOverride(new Color(249, 255, 214, 255));
		}
	...
	public void convertDrones(ShipAPI ship, final ShipAPI target) {
		...
		for (ShipAPI drone : drones) {
			if (num < getNumToFire()) {
				...
				if (target != null && missile.getAI() instanceof GuidedMissileAI ai) {
					ai.setTarget(target);
				}
				...
				drone.setWing(null);
				drone.setExplosionFlashColorOverride(new Color(255, 222, 222, 255));
				...
				EmpArcEntityAPI arc = engine.spawnEmpArcVisual(ship.getLocation(), ship, missile.getLocation(), missile, thickness, new Color(255, 170, 170, 255), Color.white);
	...
	protected ShipAPI findTarget(ShipAPI ship) {
		...
		if (target != null && target.isFighter()) {
			ShipAPI nearbyShip = Misc.findClosestShipEnemyOf(
					ship,
					target.getLocation(),
					HullSize.FRIGATE, 100, false);
